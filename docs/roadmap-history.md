...
**Current addition:** Linked Updates page and change-note maintenance checks are implemented and locally verified; publication remains part of FND-004.
...
| FND-005 | P1       | Add linked updates and enforce change-note maintenance         | HELM 1.0.x        | Done      | Firas | 2026-09-17          | `docs/updates.md`; 13 unit tests, update coverage and generated-content checks pass; timeline/navigation/no-JavaScript browser checks verified locally |
...
### 2026-09-17 — Keep updates linked and single-source

Use one structured registry for the Updates page, homepage highlights, published-updates RSS and generated changelog. Every change item links to its destination. Require meaningful update notes for visitor-facing diffs and a matching entry for the current release/state. Mark candidates visibly and announce only published entries through RSS.
...
- Started the linked updates mechanism as FND-005, with one registry, generated release notes, subscriber feed, and CI coverage for future changes.
- Completed FND-005 locally: `/updates`, homepage highlights, linked historical/current notes, published-only RSS, required release metadata, generated changelog checks, and meaningful change-note coverage against the PR base.

...
- 2026-09-17: Site-wide editorial and coherence pass, recorded in `editorial-review-2026-09-17.md`. Current public copy uses the five agreed shared competencies, describes role responsibilities without fixed career predictions, distinguishes instructions from enforced access, and qualifies adoption/maturity/measurement advice at the point of use. The exact least-member formula is removed from the practitioner and leadership narratives and Engineering Manager responsibilities. Staffing counts are explicitly illustrative; the larger example's lower bound is corrected to match its listed roles. The decision table retains its listed roles as starting points for naming one owner locally. Test-coverage, rule-update and review-rejection directions now say to monitor rather than assume an increase/decrease is improvement. Stable identifiers and numeric measures are retained; prior scores must not be reinterpreted using the revised prose. These interpretation changes need release-level compatibility review and do not close the broader validation work.
